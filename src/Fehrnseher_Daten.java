...
public class Fehrnseher_Daten {
	...
	public void saveAsFile() {
		...
		try {
			...
			/*
			 * for (int i = 0; i < this.kanalAnzahl - 1; i++) { if
			 * (kanalReihenfolge[i] != "") { this.kanalReihenfolge[i] += ";";
			 * byte[] contentInBytes = this.kanalReihenfolge[i].getBytes();
			 * fop.write(contentInBytes); } // end of if } // end of for byte[]
			 * contentInBytes = this.kanalReihenfolge[this.kanalAnzahl -
			 * 1].getBytes(); //verhindern das der Letzte Eintrag auch mit ;
			 * endet. fop.write(contentInBytes);
			 */
			fop.flush();
			fop.close();
		} catch (IOException e) {
	...
	public void readFromFile() {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(
			...
			if (lines.size() >= 5) {
				...
				this.seitenverhaeltnis = Integer.parseInt(lines.get(3)); // problem
																			// //
																			// Luka:
																			// ????
		...
		for (int i = 0; i < lines2.size(); i++) {
			...
			programm = Channel[3];
			anbieter = Channel[4];
			// kanal tmp = new kanal(frequenz, kanal, qualitaet, programm,
			// anbieter);
